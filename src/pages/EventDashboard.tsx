...
export function EventDashboard() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-6xl">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-3 text-xs text-text-muted">
              ...
              <Badge variant={event.visibility === 'public' ? 'cyan' : 'pink'} size="sm">{event.visibility}</Badge>
            </div>
          </div>
      ...
      {/* Tab bar */}
      <div className="flex items-center gap-1 p-1 rounded-2xl bg-white/[0.04] border border-white/10 w-fit mb-6 overflow-x-auto">
        {tabs.map(tab => (
          <button
            key={tab.key}
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <Card variant="glass" padding="md" glow="yellow">
              <CardContent>
                <p className="text-2xl font-bold text-white">{totalSold}/{totalCapacity}</p>
                <p className="text-xs text-text-muted">Tickets Sold</p>
              </CardContent>
            </Card>
            <Card variant="glass" padding="md" glow="pink">
              <CardContent>
                <p className="text-2xl font-bold text-white">{orders.length}</p>
                <p className="text-xs text-text-muted">Orders</p>
              </CardContent>
            </Card>
            <Card variant="glass" padding="md" glow="cyan">
              <CardContent>
            ...
            <Card variant="glass" padding="md" glow="yellow">
              <CardContent>
                <p className="text-2xl font-bold text-white">{checkedIn}/{totalSold}</p>
        ...
        <div className="flex items-center gap-3 mb-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 bg-white/[0.04] border border-white/10 rounded-xl text-white text-sm placeholder:text-text-muted focus:outline-none focus:border-yellow-400/50"
            />
          </div>
        ...
        {attendeeRows.length > 0 ? (
          <div className="space-y-2">
            {attendeeRows.map(({ order, ticket }) => (
              <div key={order.id} className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/5">
        ...
        {tickets.length > 0 ? (
          <div className="space-y-2">
            {tickets.map(ticket => {
              ...
              return (
                <div key={ticket.id} className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/5">
  ...
  return (
    <div className="space-y-4">
      ...
      {ticketTypes.length > 0 && (
        <Card variant="glass" padding="md">
          <CardContent>
            <p className="text-sm font-semibold text-white mb-3">Per Ticket Type</p>
            <div className="space-y-2">
              {ticketTypes.map(tt => {
                ...
                return (
                  <div key={tt.id} className="p-3 rounded-xl bg-white/[0.02] border border-white/5">
                    ...
                    <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
                      <div className="h-full bg-gradient-to-r from-yellow-400 to-cyan-400 rounded-full" style={{ width: `${ttPct}%` }} />
                    </div>
      ...
      {totalCapacity > 0 && (
        <Card variant="glass" padding="md">
          <CardContent>
            <p className="text-sm font-semibold text-white mb-3">Overall Capacity</p>
            <div className="h-3 bg-white/5 rounded-full overflow-hidden">
              <div
                className={cn(
                  'h-full rounded-full transition-all',
                  sellRate >= 100 ? 'bg-red-500' : 'bg-gradient-to-r from-yellow-400 via-accent-pink to-cyan-400'
                )}
...
function ScannerTab({
  ...
  return (
    <Card variant="glass" padding="lg">
      <CardContent>
        <p className="text-sm font-semibold text-white mb-3">Verify Ticket</p>
        <div className="flex gap-2 mb-4">
          <input
            type="text" placeholder="Enter or scan QR code..." value={scanInput}
            onChange={e => setScanInput(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && onScan()}
            className="flex-1 px-4 py-2.5 bg-white/[0.04] border border-white/10 rounded-xl text-white text-sm placeholder:text-text-muted focus:outline-none focus:border-yellow-400/50"
          />
        ...
        <div className="mt-4 p-3 bg-yellow-400/10 border border-yellow-400/20 rounded-xl text-xs text-yellow-400/80">
